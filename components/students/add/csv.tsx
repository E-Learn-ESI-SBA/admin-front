  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const data = lines.slice(1).map((line, index) => {
        const [name, classYear,email, gender,city, phoneNumber] = line.split(',');
        return {
          name,
          class: classYear,
          email,
  ...
  return (
      <form className='flex flex-col gap-8 p-4'  onSubmit={handleSubmit(onSubmit)}>
        ...
        {csvData.map((item, index) => (
          <li key={index}>
            Name: {item.name}, Class: {item.class}, Gender: {item.gender}, Email: {item.email},city: {item.city} ,Phone Number: {item.phone_number}
          </li>
        ))}
      </ul>
